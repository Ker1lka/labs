...
Горизонтальне масштабування (*scale-out*) дозволяє нарощувати загальну потужність системи шляхом поступового додавання звичайних, бюджетних серверів, а також гнучко відключати їх під час спаду трафіку для оптимізації витрат. Вертикальне масштабування (*scale-up*) швидко впирається у фізичні обмеження заліза (максимум ядер або оперативної пам'яті на один сервер), де кожен наступний крок модернізації коштує експоненціально дорожче. До того ж вертикальний апгрейд майже завжди вимагає тимчасової зупинки системи (*downtime*) та не захищає систему від повної відмови, якщо цей один потужний сервер вийде з ладу.

### 2. Що станеться з монолітом із in-memory сесіями, якщо запустити його у 3 екземплярах за балансувальником?

Користувач зіткнеться з хаотичною та нестабільною роботою вебдодатка через постійну втрату стану авторизації та сесійних даних. По-перше, після успішного логіну на першому екземплярі, наступний клік користувача може бути перенаправлений балансувальником на другий екземпляр, який нічого не знає про цю сесію, через що сайт знову викине користувача на сторінку авторизації. По-друге, будь-які накопичувальні дані (наприклад, товари в кошику покупця), що зберігаються в пам'яті процесу, будуть постійно зникати або змінюватися залежно від того, на яку саме репліку балансувальник направить черговий HTTP-запит.

### 3. Які керовані сервіси AWS / Azure / GCP виконують роль зовнішнього сховища стану (як Redis у цій лабі)?

Для забезпечення високої доступності стану без необхідності адміністрування інфраструктури хмарні провайдери пропонують такі повністю керовані сервіси:
* **AWS:** *Amazon ElastiCache for Redis* (або *Amazon MemoryDB for Redis* для гарантованої стійкості даних).
